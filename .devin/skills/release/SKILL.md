...
## Prerequisites
1. Identify the version to document (user will specify, e.g., "0.1", "0.2")
2. Check `docs/releases/` for the latest release notes format examples, if any exist
3. Determine the GitHub milestone or project board associated with this version
4. Get GitHub repository details (owner: captainkirk99, repo: nisar_play)

## Steps

### 1. Gather Issues and PRs for Version
Use the `gh` CLI (preferred over MCP tools on this network) to collect:
...
gh issue list --state closed --search "milestone:v{VERSION}"

# Search for merged PRs in version
gh pr list --state merged --search "milestone:v{VERSION}"

# Alternative: Search by labels if no milestone
gh issue list --state closed --label "v{VERSION}"
```
...
- **Environment**: venv, project yaml/requirements changes (label: build)
- **Testing**: Test coverage, new tests (label: testing)
- **Documentation**: Docs updates (label: documentation)
- **Dependencies**: New or updated Python packages
- **Data Handling**: NISAR file reading, plotting, data products
...
Examine the most recent release notes in `docs/releases/` (if any exist; otherwise use the structure below) to understand:
...
- **Features**: Organized by functional areas with detailed descriptions
  ...
  - Group by: data reading, plotting/visualization, environment setup, etc.
- **Bug Fixes**: List significant bugs resolved with issue numbers
  - Format: "**[Bug description]** (#456) - Resolution details"
- **Data Handling**: New/modified NISAR file reading or plotting capabilities (if applicable)
- **Environment**: venv and project yaml/requirements changes, new configuration options
- **Testing**: Coverage metrics, test strategies, new test files added
- **Documentation**: README, docstrings, user guides (if applicable)
- **Dependencies Added**: New Python packages (h5py, netCDF4, xarray, matplotlib, etc.) with versions
...
```
User: "Create release notes for version 0.1"
→ Search GitHub for milestone:v0.1 issues and PRs
→ Categorize by labels (enhancement, bug, build, etc.)
→ Generate `docs/releases/v0.1.md`
...
```
User: "Generate release notes for v0.2"
→ Search for label:v0.2 closed issues
→ Search for merged PRs with v0.2 label
→ Extract implementation details from PR descriptions
→ Group by functional area (data reading, plotting, environment, etc.)
